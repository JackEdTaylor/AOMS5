...
As we saw briefly at the [end of the last section](05-power_analyses#modelling-statistical-power), one common solution is to use a logit link. Here, we transform probabilities into "logistic units", or *logits*.
...
Instead, we simulate such noise in the mapping from probabilities, `Pr_{ip}`, to observed binary outcomes or responses, $Y_{ip}$:
...
```{r}
trials2 <- sim_trials(b_0=0.4, s_I0=0.5, s_P0=0.5, s_P1=0.25, n_items=150, n_participants=90)
...
What about when, rather than two possible responses (e.g., correct/incorrect), we want to model an ordinal variable. A common example might be studies where we ask participants to rate items. In this case there are powerful extensions that build on the logic of the Bernoulli/binomial model to describe ordinal responses [@burknerOrdinalRegressionModels2019].
