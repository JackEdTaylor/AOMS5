...
This tells us that if we present 50 participants with 50 items, and the effect of Zipf is -0.033, then in the long run we should expect `{r} round(mean(sims_df$p.value < 0.05) * 100, 2)`% of studies to correctly detect the effect using our analysis method.
...
Here we calculate the power for each number of participants. I also calculate the a binomial confidence interval for each value of `n_participants`.
...
We can now plot how expected statistical power varies as we change the number of participants. I also add a horizontal line to show where 90% power would be:
...
We could even predict the number of participants required to reach a certain power target. For example, if we want to reach 90% power, we can rearrange the formula for a logit-link binomial model to calculate the N associated with a probability of 0.9.
...
Based on this, we might plan to collect from 5 participants to reach the power target.
...
2. Run new simulations to consider possible values of both N Participants and N Items. I suggest 100 simulations per parameter. If possible, try to recreate the plot below:
...
3. The plot above actually fit a separate model for each value of `n_items`. Can you think of a way of fitting one model that uses both `n_items` and `n_participants` as predictors? If possible, see whether you can produce the two plots below.
